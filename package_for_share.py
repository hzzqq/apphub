...
# 排除的目录（任意层级）—— 这些是「非应用目录」，没有 index.html，只服务开发，不应发给用户
#   test/        各 App 与大厅的前端单测（含断言细节）
#   deploy/      服务器部署脚本与 systemd 单元（含主机路径）
#   Artifacts/   开发过程文档
EXCLUDE_DIRS = {".workbuddy", "__pycache__", ".pytest_cache", ".git", "node_modules",
                "Artifacts", "test", "deploy"}
# 排除的文件名（任意层）
EXCLUDE_FILES = {
...
这是一个「30 个零依赖单文件网页应用 + 真实数据后端」的打包。
