// 自动生成 (tools/gen_microapps.py) — AI 投研问答
...
runAppTest(__dirname, ({ sandbox, ok, eq, arrEq, src, err }) => {
  ...
  ok("POST 助手 fetchT/showError 存在", src.indexOf("fetchT(")>=0 && src.indexOf("function showError")>=0);ok("多轮 QA_HISTORY 存在", src.indexOf("QA_HISTORY")>=0);ok("按模型隔离历史键 qaKey 存在", src.indexOf("function qaKey")>=0 && src.indexOf("qa_history_")>=0);ok("导出对话 exportChat 存在", src.indexOf("function exportChat")>=0 && src.indexOf("对话导出.txt")>=0);
});
