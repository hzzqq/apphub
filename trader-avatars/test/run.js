...
runAppTest(__dirname, ({ sandbox, ok, eq, arrEq, src, err }) => {
  ...
  ok("POST 助手 fetchT/showError 存在", src.indexOf("fetchT(")>=0 && src.indexOf("function showError")>=0);ok("人格列表 TRAIT_PERSONAS 存在", src.indexOf("TRAIT_PERSONAS")>=0);ok("多轮 QA_HISTORY 存在", src.indexOf("QA_HISTORY")>=0);ok("按人格隔离历史键 qaKey 存在", src.indexOf("function qaKey")>=0 && src.indexOf("qa_history_")>=0);
});
