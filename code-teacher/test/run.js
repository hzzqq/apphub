...
const { sandbox, src, err, doc, window, localStorage } = loadApp(__dirname, { window: false });
...
// ============ 1. 词法 ============
const t1 = sandbox.tokenizeToLines("x = 5\nprint(x)");
...
// ============ 2. 语法 ============
const ast1 = sandbox.parse(sandbox.tokenizeToLines("x = 5\nprint(x)").lines);
...
const exPlain = sandbox.explainProgram("x = 5\nprint(x)", "plain");
ok(/赋值/.test(exPlain), "讲解：plain 含赋值说明");
const exMom = sandbox.explainProgram("for i in range(3):\n    print(i)", "mom");
ok(/循环|小火车|苹果/.test(exMom), "讲解：mom 含生活类比");
const exBro = sandbox.explainProgram("def add(a,b):\n    return a+b", "brother");
ok(/魔法盒子|小狐狸/.test(exBro), "讲解：brother 含童趣拟人");

// 关键回归：旧 classify 对 `if x>0`（无 else）会误判 default；新版按 AST 识别 if
const exIf = sandbox.explainProgram("if x>0:\n    print('pos')", "plain");
ok(/判断|条件/.test(exIf), "讲解：无 else 的 if 仍被正确识别");

// ============ 4. 错误检测：未定义变量 ============
const errSrc = "print(score)\nscore = 100";
const errAst = sandbox.parse(sandbox.tokenizeToLines(errSrc).lines);
const errs = sandbox.findErrorsWrapper(errSrc, errAst, []);
ok(errs.some((e) => /未定义/.test(e.msg)), "纠错：检测到未定义变量 score");

// 干净代码应报告“没问题”
const cleanAst = sandbox.parse(sandbox.tokenizeToLines("x = 1\nprint(x)").lines);
const cleanErrs = sandbox.findErrorsWrapper("x = 1\nprint(x)", cleanAst, []);
ok(cleanErrs.length === 1 && cleanErrs[0].severity === "ok", "纠错：干净代码报 ok");

// ============ 5. 错误检测：条件里写 = 而非 == ============
const eqAst = sandbox.parse(sandbox.tokenizeToLines("if x = 5:\n    pass").lines);
const eqErrs = sandbox.findErrorsWrapper("if x = 5:\n    pass", eqAst, []);
ok(eqErrs.some((e) => /==/.test(e.msg)), "纠错：if 里写成 = 被指出应用 ==");

// ============ 6. 复杂度分析 ============
const cSingle = sandbox.analyzeComplexity(sandbox.parse(sandbox.tokenizeToLines("for i in range(3):\n    print(i)").lines));
eq(cSingle.depth, 1, "复杂度：单层循环 depth=1");
ok(/O\(n\)/.test(cSingle.label), "复杂度：单层循环标 O(n)");

const cDouble = sandbox.analyzeComplexity(sandbox.parse(sandbox.tokenizeToLines("for i in range(3):\n    for j in range(3):\n        print(i)").lines));
eq(cDouble.depth, 2, "复杂度：双层循环 depth=2");
ok(/O\(n²\)/.test(cDouble.label), "复杂度：双层循环标 O(n²)");

const cFlat = sandbox.analyzeComplexity(sandbox.parse(sandbox.tokenizeToLines("x = 1\ny = 2\nprint(x)").lines));
eq(cFlat.depth, 0, "复杂度：无循环 depth=0");
ok(/O\(1\)/.test(cFlat.label), "复杂度：无循环标 O(1)");

// 递归检测
const cRec = sandbox.analyzeComplexity(sandbox.parse(sandbox.tokenizeToLines("def f(n):\n    return f(n-1)").lines));
ok(cRec.notes.some((n) => /递归/.test(n)), "复杂度：检测到递归");

// 列表推导式应计为一次循环（旧实现会误判 O(1)）
const cComp = sandbox.analyzeComplexity(sandbox.parse(sandbox.tokenizeToLines("squares = [x*x for x in range(3)]").lines));
ok(cComp.depth >= 1 && /O\(n\)/.test(cComp.label), "复杂度：列表推导式标 O(n)");
const cCompNest = sandbox.analyzeComplexity(sandbox.parse(sandbox.tokenizeToLines("for i in range(3):\n    s = [x*x for x in range(3)]").lines));
ok(/O\(n²\)/.test(cCompNest.label), "复杂度：for 内套列表推导式标 O(n²)");

// ============ 7. 执行：for + print ============
const r1 = sandbox.execute("for i in range(3):\n    print(i)");
eq(r1.output, "0\n1\n2", "执行：for+print 输出 0 1 2");

// ============ 8. 执行：函数定义 + 调用 + 返回值 ============
const r2 = sandbox.execute("def add(a, b):\n    return a + b\nprint(add(2, 3))");
eq(r2.output, "5", "执行：函数 add(2,3) 输出 5");

// ============ 9. 执行：if/else ============
const r3 = sandbox.execute("x = 10\nif x > 0:\n    print('pos')\nelse:\n    print('neg')");
eq(r3.output, "pos", "执行：if 分支命中 pos");
const r3b = sandbox.execute("x = -1\nif x > 0:\n    print('pos')\nelse:\n    print('neg')");
eq(r3b.output, "neg", "执行：else 分支命中 neg");

// ============ 10. 执行：列表 + sum + 算术 ============
const r4 = sandbox.execute("numbers = [1, 2, 3]\nprint(sum(numbers))");
eq(r4.output, "6", "执行：sum([1,2,3]) 输出 6");

// ============ 11. 执行：增强赋值 ============
const r5 = sandbox.execute("n = 1\nn += 4\nprint(n)");
eq(r5.output, "5", "执行：n+=4 输出 5");

// ============ 12. 执行：while 循环 ============
const r6 = sandbox.execute("i = 0\nwhile i < 3:\n    print(i)\n    i += 1");
eq(r6.output, "0\n1\n2", "执行：while 输出 0 1 2");

// ============ 13. 执行：字符串 / 列表运算 ============
const r7 = sandbox.execute('s = "ab"\nprint(s * 3)');
eq(r7.output, "ababab", "执行：字符串乘法");
const r8 = sandbox.execute("a = [1,2]\nb = [3,4]\nprint(a + b)");
eq(r8.output, "1,2,3,4", "执行：列表相加");

// ============ 14. 执行：方法（append / upper） ============
const r9 = sandbox.execute("lst = []\nlst.append(1)\nlst.append(2)\nprint(lst)");
eq(r9.output, "1,2", "执行：list.append");
const r10 = sandbox.execute('s = "hello"\nprint(s.upper())');
eq(r10.output, "HELLO", "执行：str.upper");

// ============ 15. 执行：未定义变量抛错（UI 会友好提示） ============
let threw = false;
try { sandbox.execute("print(y)"); } catch (e) { threw = true; }
ok(threw, "执行：未定义变量抛出错误（交由 UI 兜底）");

// ============ 16. 执行：死循环防护 ============
let guardThrew = false;
try { sandbox.execute("while True:\n    pass"); } catch (e) { guardThrew = /死循环/.test(e.message); }
ok(guardThrew, "执行：死循环被 STEP_CAP 拦下");

// ============ 17. 端到端 analyze 聚合 ============
const full = sandbox.analyze("for i in range(5):\n    print(i)", "plain");
...
// ============ 18. 列表推导式：解析 / 讲解 / 执行 ============
const lcAst = sandbox.parse(sandbox.tokenizeToLines("squares = [x*x for x in range(3)]").lines);
eq(lcAst.body[0].type, "assign", "推导式：顶层仍是赋值");
const lcExec = sandbox.execute("squares = [x*x for x in range(3)]\nprint(squares)");
eq(lcExec.output, "0,1,4", "推导式：平方列表 [0,1,4]");
const lcExec2 = sandbox.execute("evens = [x for x in range(6) if x % 2 == 0]\nprint(evens)");
eq(lcExec2.output, "0,2,4", "推导式：带 if 过滤 [0,2,4]");
const lcEx = sandbox.explainProgram("a = [x for x in range(3)]", "plain");
ok(/列表推导式/.test(lcEx), "推导式：讲解识别列表推导式");

// ============ 19. 切片 / 负索引 ============
const slExec = sandbox.execute("lst = [10,20,30,40]\nprint(lst[1:3])\nprint(lst[-1])");
eq(slExec.output, "20,30\n40", "切片/负索引：lst[1:3]=[20,30], lst[-1]=40");

// ============ 25. 执行：break / continue ============
const rb = sandbox.execute("i = 0\nwhile i < 5:\n    i += 1\n    if i == 3:\n        break\n    print(i)");
eq(rb.output, "1\n2", "执行：break 跳出循环（只打印 1,2）");
const rc = sandbox.execute("for i in range(5):\n    if i % 2 == 0:\n        continue\n    print(i)");
eq(rc.output, "1\n3", "执行：continue 跳过偶数（打印 1,3）");

// ============ 24. 练习自测比对 ============
const ce1 = sandbox.checkExercise("0\n1\n2", "0\n1\n2");
ok(ce1.pass === true, "自测：输出一致判通过");
const ce2 = sandbox.checkExercise("15", "0");
ok(ce2.pass === false, "自测：输出不一致判不通过");
const ce3 = sandbox.checkExercise("yes\n", "yes");
ok(ce3.pass === true, "自测：容忍首尾空白");
const ceSol = sandbox.execute("for i in range(3):\n    print(i)");
ok(sandbox.checkExercise("0\n1\n2", ceSol.output).pass, "自测：正确解通过比对");

// ============ 21b. 结构检测：缺冒号 ============
const structTok = sandbox.tokenizeToLines("if x > 0\n    print('pos')").lines;
const structErrs = sandbox.findStructureIssues(structTok);
ok(structErrs.some(function(e){ return /冒号/.test(e.msg); }), "结构：if 缺冒号被指出");
const structOk = sandbox.findStructureIssues(sandbox.tokenizeToLines("if x > 0:\n    print('pos')").lines);
ok(structOk.length === 0, "结构：if 有冒号不误报");
const structFor = sandbox.findStructureIssues(sandbox.tokenizeToLines("for i in range(3)\n    print(i)").lines);
ok(structFor.length === 1, "结构：for 缺冒号被指出");
const structAnalyze = sandbox.analyze("def add(a, b)\n    return a+b", "plain");
ok(structAnalyze.errors.some(function(e){ return /冒号/.test(e.msg); }), "结构：analyze 聚合能返回缺冒号问题");

// ============ 21. 多变量赋值 / 解包 ============
const ma = sandbox.execute("a, b = 1, 2\nprint(a)\nprint(b)");
eq(ma.output, "1\n2", "多变量赋值：a=1, b=2");
const ma2 = sandbox.execute("x, y = [10, 20]\nprint(x + y)");
eq(ma2.output, "30", "列表解包：x,y=[10,20] → 30");
const ma3 = sandbox.execute("one, two, three = range(3)\nprint(one + two + three)");
eq(ma3.output, "3", "可迭代解包：range(3) 拆给三个变量");

// ============ 20. 运行轨迹（教学价值） ============
const tr = sandbox.execute("x = 2\ny = x * 3\nprint(y)", {trace:true});
...
const trLoop = sandbox.execute("for i in range(3):\n    print(i)", {trace:true});
