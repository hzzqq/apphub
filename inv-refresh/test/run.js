// 自动生成 (tools/gen_microapps.py) — 库存刷新台
...
runAppTest(__dirname, ({ sandbox, ok, eq, arrEq, src, err }) => {
  ...
  ok("端点已配置 (/api/refresh)", src.indexOf("/api/refresh") >= 0);
  ok("渲染函数存在", src.indexOf("function render") >= 0);
  ok("定制渲染钩子存在", src.indexOf("function renderCustom") >= 0);
  ok("模板占位符已替换", src.indexOf("__CUSTOM") < 0);
  ok("POST 助手 fetchT/showError 存在", src.indexOf("fetchT(")>=0 && src.indexOf("function showError")>=0);ok("批量锁 BATCH_LOCK 存在", src.indexOf("BATCH_LOCK")>=0);
});
