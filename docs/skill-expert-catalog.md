...
- 双击根目录 **`verify.bat`**，或命令行 `python verify_all.py` → **12 维门禁**一键校验（前端语法/单测/运行时/后端25端点/数据一致性/目录卫生/端点一致性/真实数据/期货保护/生成守卫/库存守卫/生态端点）。详见 `docs/verify.md`。
- push / PR 时由 `.github/workflows/verify.yml`（GitHub Actions）自动跑。

## 附：分派台账（照着跑即可）

> 用法：把「分派 prompt」贴给对应 Skill/专家。P0 先跑。

| ID | 板块 | 任务 | Skill / 专家 | 分派 prompt | 优先级 | 频率 |
|---|---|---|---|---|---|---|
| T01 | 期货与产业链 | 6 品种早报 | `futures-combo` | 用 futures-combo 出玻璃FG/纸浆SP/纯碱SA/鸡蛋JD/白糖SR/乙二醇EG 的早报 | P0 | 每日盘前 |
| T02 | 行情与报价 | 自选行情速览 | `westock-data` | 用 westock-data 拉我自选股实时价/涨跌/量，红涨绿跌表格 | P0 | 每日 |
| T03 | 多智能体投研 | 多空协同研判 | `ai-hedge-fund` 团队 | 对 XX 标的并行基本面/技术/估值/情绪/风控，最后给 BUY/SELL/HOLD | P0 | 按需 |
| T04 | 板块与轮动 | 板块选择 | 专家 `sector-screening` | 当前最值得关注的 3 个板块 + 核心标的，给理由 | P1 | 每周 |
| T05 | 个股与持仓 | 持仓体检 | 专家 `portfolio-diagnosis` | 诊断我的持仓：风险+泡沫+拥挤度+北向态度 | P1 | 每周 |
| T06 | 风险与情绪 | 情绪面复盘 | `a-share-daily-review` | 输出今日情绪面：涨停梯队+资金流+情绪打分 | P1 | 每日收盘 |
| T07 | 工程与交付 | 锐评迭代 | `silent-failure-defect-hunt` | 对 App Hub 做 N 轮：每轮找一真缺陷→修复→验证→提交 | P1 | 每轮 |
| T08 | 数据可观测 | 数据源体检 | （数据专家） | 检查后端各数据源覆盖与新鲜度，列出降级/断点 | P2 | 每周 |
| T09 | 创意·3D·主题 | 主题扩色 | `ui-ux-pro-max` | 给主题工坊新增 3 套配色并导出配置 | P2 | 按需 |
| T10 | 生活与效率 | UI 精修 | `frontend-design` | 给「行程规划」做一版更精致的零依赖单文件 UI | P2 | 按需 |
